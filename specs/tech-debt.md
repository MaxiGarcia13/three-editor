...
- [ ] Remove unused `loadModel` in `src/modules/viewport/stores/model-store.ts` (alias of `importModelFiles([file])`) and drop it from `src/modules/viewport/index.ts`
- [ ] Remove unused `resetModel` (clears the whole library; no caller) and drop it from the viewport barrel
