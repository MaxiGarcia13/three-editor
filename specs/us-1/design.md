...
Character load + viewport shell only. No mixer, clip library, or exporter.

## Structure

- Astro page hydrates one editor React island (`client:load` or equivalent)
- `editor-shell`: layout, collapsible sidebar, file input for character
- `viewport`: R3F `Canvas`, lights, ground/helpers as needed, OrbitControls, character scene graph once loaded
...
4. Replace any previously loaded character (single character at a time)

## UI

- Full-bleed canvas
- Collapsible sidebar with character upload control and empty/error copy
